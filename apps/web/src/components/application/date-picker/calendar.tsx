...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // Patched for react-aria-components ≥ 1.21 (multi-select calendars pass arrays); single-select context keeps the first value.
    const onChange = (next: DateValue | readonly DateValue[] | null) => setValue(Array.isArray(next) ? ((next[0] as DateValue | undefined) ?? null) : (next as DateValue | null));
